...
# 
# 
# - In our case, we generated 1000 median correlation scores from randomly combined replicates as the **null distribution** for each no_of_replicates/replicate class WITHOUT regards to dose (the default)
# 
...
def get_cpds_replicates(df, df_lvl4):
    ...
    for cpd in df.index.unique():
        replicate_names = df_lvl4[df_lvl4['pert_iname'] == cpd]['replicate_name'].values.tolist()
...
null_distribution_replicates = get_null_distribution_replicates(
    cpd_replicate_class_dict, replicates_in_all, cpds_replicates, rand_num=1000
)
...
save_to_pickle(null_distribution_replicates, cp_level4_path, 'null_distribution_dose_independent.pickle')
...
with open(os.path.join(cp_level4_path, 'null_distribution_dose_independent.pickle'), 'rb') as handle:
...
save_to_pickle(dose_null_medians, cp_level4_path, 'null_dist_medians_per_dose_dose_independent.pickle')
...
save_to_csv(df_null_p_vals.reset_index().rename({'index':'cpd'}, axis = 1), cp_level4_path, 
            'cpd_replicate_p_values_dose_independent.csv')


# In[37]:


cpd_summary_file = pathlib.Path(cp_level4_path, 'cpd_replicate_p_values_melted_dose_independent.csv')
